...
void EEprom_write_all() { 
	...
	if ((tempbyt == 0) || (tempbyt == 1) || (EEPROM_clear)) { EEPROM.put((4 + sizeof(GPSSet)), GPSSet); Serial.print("rewriting EEPROM + write 2. set at #"); Serial.println(4 + sizeof(GPSSet)); }//write 2. time with defaults to be able to reload them  
	delay(2);
	EEPROM.commit();
}
//--------------------------------------------------------------
...
/*	while (Serial2.available())
	{
		...
		if (UBXDigit2 < 2) {
			...
			if (isUBXRelPosNED) { 
				...
				if (debugmodeUBX) { Serial.print("UBX2 digit  "); Serial.print(UBXDigit2); Serial.print(" incom Byte: "); Serial.println(incomByte2); }
			...
			if (UBXDigit2 == 5) {
				if (UBXPVT2[nextUBXcount2].cls == 0x01)
				{
					if (UBXPVT2[nextUBXcount2].id == 0x07) {
						//PVT sentence
						isUBXPVT2 = true;
						if (debugmodeUBX) { Serial.println("UBX PVT2 started on serial 2"); }
					}
					else 
					{
						isUBXPVT2 = false;
						if (UBXPVT2[nextUBXcount2].id == 0x3C) {
							...
							if (debugmodeUBX) { Serial.println("UBX RelPosNED started on serial 2"); }
						}
				...
				if (UBXDigit2 == 7) {//lenght
					...
					if (debugmodeUBX) { Serial.print("UBXLenght2: "); Serial.println(UBXLenght2); }
				}
				else
				{
						...
						if (isUBXRelPosNED) {//compare checksum
							...
							if (isUBXPVT2) {//compare checksum
								if ((UBXPVT2[nextUBXcount2].CK0 == UBXchecksum2[0]) && (UBXPVT2[nextUBXcount2].CK1 == UBXchecksum2[1])) {
									...
									isUBXPVT2 = false;
									UBXLenght1 = 100;
									if (debugmodeUBX) { Serial.println("UBX2 RelPosNED checksum invalid"); }
								}
							}//PVT
						}//else RelPosNED
