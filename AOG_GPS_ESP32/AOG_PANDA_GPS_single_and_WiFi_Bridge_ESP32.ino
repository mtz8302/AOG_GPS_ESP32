...
byte vers_nr = 57;
char VersionTXT[150] = " - 16. Apr 2023 by MTZ8302<br>PANDA (single GPS only, BNO085 RVC serial, CMPS14 via I2C, WiFi to Ethernet bridge, multiple WiFi networks)";
...
//change stettings to your need. Afterwards you can change them via webinterface x.x.x.79 (192.168.1.79 or 192.168.137.79)
//if connection to your network fails an accesspoint is opened: connect to it via WiFi and get to webinterface 192.168.137.1
...
//So if changing settings set EEPROM_clear = true; (line ~118) - flash - boot - reset to EEPROM_clear = false - flash again to keep them as defauls

struct set {
    //connection plan:
    //  ESP32 --- F9P GPS pos ---Sentences ---- BNO085 (Adafruit, RVC mode)
    //  RX1-27-------TX1-----------NMEA GGA and VTG out   (=position+speed)
    //  TX1-16-------RX1----------------RTCM in           (NTRIP comming from AOG to get absolute/correct postion
    //  RX2-25-------------------------------- TX = SDA
    //  TX2-17-------------------------------- no serial input at BNO

    // IO pins ESP32 side ----------------------------------------------------------------------------
    //ESP32 dev modul 27 16 25 17 LED 2
    //PANDA ESP32 dev pico D4 board v1.2 27 14 25 26 int 10 LED 15 (v1.0 27 14 26 25 int 10 LED 15)
    ...
    byte Button_WiFi_rescan_PIN = 255;  //Button to rescan/reconnect WiFi networks / push to GND
    ...
    char ssid3[24] = "Fendt_260V_Tablet";                     // WiFi network Client name
    char password3[24] = "Fendt_260V";                 // WiFi network password//Accesspoint name and password
    char ssid4[24] = "Deutz_6006_Tablet";                     // WiFi network Client name
    char password4[24] = "Deutz_6006";                 // WiFi network password//Accesspoint name and password
    char ssid5[24] = "";                     // WiFi network Client name
    char password5[24] = "";                 // WiFi network password//Accesspoint name and password

    char ssid_ap[24] = "AgOpenGPS_net";  // name of Access point, if no WiFi found, or no SSID1 .. SSID5 set !!
    char password_ap[24] = "";     // password for Access point, if no WiFi found, or no SSID1 .. SSID5 set !!

    int timeoutRouterWiFi = 20;                 // time (s) to search for existing WiFi, then starting Accesspoint 
    int timeoutRouterEth = 120;                  // time (s) to search for cable Ethernet connection, when time has passed changing to WiFi data transfer

    byte timeoutWebIO = 255;                  // time (min) afterwards webinterface is switched off

//WiFi
    byte WiFi_myip[4] = { 192, 168, 137, 79 };   // Roofcontrol module 
...
//WIFI+Ethernet
unsigned long now = 0, NtripDataTime = 0, NtripDataMsgTime = 0, WebIOLastUsePlus3 = 0, WebIOTimeOut = 0, WiFi_network_search_timeout = 0, Eth_network_search_timeout = 0, oneSecLoopTime = 0;//WiFi_lost_time = 0,
...
bool  WebIORunning = false, WiFiUDPRunning = false, task_WiFiConnectRunning = false, task_EthConnectRunning = false;
...
const byte IMUDataTimeShiftCMPS = 9, IMUDataTimeShiftBNO = 9;//+9 = -1 as ring of 10; take older data for PANDA sentence, as GPS data is about 80ms old and IMU data is 100Hz -> 4=40ms
const byte IMUBNOreadTime = 20, IMUCMPSreadTime = 49; //time ms to read IMU 
...
void setup()
{
    ...
    if (Set.Button_WiFi_rescan_PIN != 255) { pinMode(Set.Button_WiFi_rescan_PIN, INPUT_PULLUP); }
    ...
    if ((strlen(Set.ssid1) == 0) && (strlen(Set.ssid2) == 0) && (strlen(Set.ssid3) == 0) && (strlen(Set.ssid4) == 0) && (strlen(Set.ssid5) == 0)) {
        ...
        Serial.println("no data (SSID) to access a WiFi network is set - starting access point");
    ...
    vTaskDelay(3000); //waiting for other tasks

    NtripDataTime = millis();
    if (!Set.DataTransVia == 7) { Set.WiFi_AOG_data_bridge = false; } //only usefull, if data transfer via Ethernet or USB
}
...
void loop()
{
    ...
    now = millis();
    if (now > IMUnextReadTime) {
        switch (Set.IMUType) {
        case 1:
            IMUnextReadTime = now + IMUBNOreadTime;
            readBNO();//serial BNO (RVC mode)            
            break;
        case 2:
            IMUnextReadTime = now + IMUCMPSreadTime;
            readCMPS();
            break;
        }        
    }//timed IMU loop
    ...
    if (Eth_connect_step == 0) {
        ...
        for (;;) {
            ...
            Eth_udpRoof.beginPacket(Eth_ipDestination, Set.PortFromAOG);
            Eth_udpRoof.write(BridgeData8888[BridgeData8888RingCountOut], BridgePacket8888Length[BridgeData8888RingCountOut]);
            Eth_udpRoof.endPacket();
            BridgeData8888RingCountOut = (BridgeData8888RingCountOut + 1) % BridgeArraySize;
            Serial.println("bridge 8888 to Eth");
        }
        //NTRIP via Ethernet
